          ...
          <a-row>
            <a-col :xs="24" :sm="12" :md="6" :lg="6" v-if="!shrinkSwitch">
              <a-form-model-item :label="$t(dict.localeObj.logline.time.range)" prop="dateRange">
                <a-select
                  allow-clear
                  v-model="loglineForm.model.dateRange"
                  size="small"
                  :placeholder="$t(dict.localeObj.logline.time.plh)"
                >
                  <a-select-option
                    :key="index"
                    v-for="(item, index) in loglineForm.external.dateRangeList"
                    :value="item.value"
          ...
          <a-row>
            ...
            <a-col :xs="24" :sm="24" :md="12" :lg="12">
              <a-form-model-item :label="$t(dict.localeObj.logline.tableColumns.time)">
                <a-input
                  size="small"
                  disabled
                  :value="timestampToDateString(loglineDetail.model.time, '-', false, false, false)"
                ></a-input>
              </a-form-model-item>
            </a-col>
            <a-col :xs="24" :sm="24" :md="24" :lg="24">
              <a-form-model-item :label="$t(dict.localeObj.logline.tableColumns.desc)">
                <a-textarea
                  :auto-size="loglineDetail.props.autoSize"
                  size="small"
                  disabled
                  v-model="loglineDetail.model.descriptor"
                ></a-textarea>
              </a-form-model-item>
            </a-col>
          </a-row>
    ...
    exportLog () {
      ...
      utils.loglineObj.getLog({
        ...
        callback: (data) => {
          ...
          utils.exportExcel(columnList, dataList, self.$t(dict.localeObj.logline.detail))
        }
      })
    },
    getLog (pageNum = 1) {
