        ...
        formObj.validate((valid) => {
          if (valid) {
            ...
            if (self.activatedKey === 1) {
              const username = formDetail[0].formModel.account.value
              const password = formDetail[0].formModel.password.value
              api.checkLoginByAccount({
                ...
                success: () => {
                  self.$message.success(
                    self.$t(dict.localeObj.loginForm.loginSuccess),
                    2
                  ).then(() => {
                    self.toManage()
                  })
              ...
              const inputCode = formDetail[0].formModel.valid.value
