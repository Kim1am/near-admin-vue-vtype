    ...
    <div class="n-common-table-title">
      <a-row>
        ...
        <a-col class="n-common-table-btn-ctl" :span="16">
          <a-button
            class="n-common-table-btn"
            :type="item.type || 'primary'"
            size="small"
            v-for="(item, index) in tableObj.btnList"
            @click="triggerBtn(item)"
            :key="index"
                  ...
                  <a-row type="flex" justify="space-between">
                    <a-col>
                      <a-checkbox
                        ...
                        v-model="checkAll"
                        size="small"
                      >{{ $t(dict.localeObj.comTable.columns.all) }}</a-checkbox>
                    </a-col>
                    <a-col>
                      <a-button
                        ...
                        type="link"
                        @click="init(true)"
                      >{{ $t(dict.localeObj.comTable.columns.reset) }}</a-button>
                    </a-col>
                  </a-row>
                </a-menu-item>
              </a-menu>
                  ...
                  <a-row type="flex" justify="space-between">
                    ...
                    <a-col class="n-pin-wrp">
                      ...
                      <a-tooltip placement="top">
                        ...
                        <a-icon
                          @click="fixedAction(index, 'cancel')"
                          v-if="item.fixed === 'right' || item.fixed === 'left'"
                          class="c-pin"
          ...
          <a-dropdown placement="bottomRight">
            ...
            <a-menu slot="overlay" v-model="sizePicked" selectable>
              <a-menu-item v-for="(item, index) in sizeMap" :key="index">{{ $t(item.name) }}</a-menu-item>
            </a-menu>
          </a-dropdown>
          <a-divider type="vertical" />
...
export default {
  ...
  methods: {
    init (isChecked) {
      const self = this
      const newList = []
      self.tableObj.columns.forEach((item) => {
        newList.push({
          ...item,
          isChecked,
    ...
    changeColumns () {
      const self = this
      const pickedList = self.selectedColumns.filter((item) => {
    ...
    fixedAction (idx, ftype) {
      const self = this
      const curPk = self.selectedColumns[idx].pk
      self.tbColumns.forEach((item, index) => {
        if (item.pk === curPk) {
          if (ftype !== 'cancel') {
            if (item.fixed === ftype) {
