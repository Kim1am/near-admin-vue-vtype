  ...
  <div class="n-tag">
    <div class="n-tag-scroll-hider">
      <span
        ...
        :key="index"
        :class="[curTagIndex === index ? 'cur' : '',saveWarning && item.params && item.params.checkSave && !item.params.isAffix ? 'n-tag-warning' : '']"
        @click="changeCp(index)"
        @contextmenu.prevent="showContextMenu($event, index)"
      >
        <i class="n-tag-affix" v-if="(item.params && item.params.isAffix)"></i>
        {{ $t(item.title) }}
        <a-icon
          v-if="!(item.params && item.params.isAffix)"
          class="n-tag-close"
          type="close"
