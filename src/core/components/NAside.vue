  ...
  <a-layout-sider class="n-layout-sider" v-model="shrinkLeftMenu" :collapsedWidth="0">
    <div class="n-aside-wrp">
      ...
      >
        <!-- if child is empty -->
        <template
          v-for="(item, index) in (menuObj.menuList[curMenu] ? (menuObj.menuList[curMenu].child || []) : [])"
        >
                    ...
                    >
                      <span slot="title" :title="$t(sitem.name)">
                        ...
                        <template v-else>
                          ...
                          <template v-else>
                            <span
                              :class="`anticon iconfont ${sitem.icon.value}`"
                              v-if="sitem.icon.value"
                            ></span>
                          </template>
                        </template>
